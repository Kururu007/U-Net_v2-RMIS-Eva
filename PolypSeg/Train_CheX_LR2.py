...
from utils.dataloader import get_loader, get_loader_CheX, test_dataset
...
def train(train_loader, model, optimizer, epoch, test_path, state):
    ...
    if (epoch + 1) % 1 == 0:
        ...
        wandb.log({f"CheX/mDice": test_macro_dice}, step=epoch)
        wandb.log({f"CheX/mIoU": test_macro_miou}, step=epoch)
        wandb.log({f"CheX/gDice": test_micro_dice}, step=epoch)
        wandb.log({f"CheX/gIoU": test_micro_miou}, step=epoch)
        wandb.log({f"CheX/Acc": test_micro_acc}, step=epoch)
...
if __name__ == '__main__':
    ...
    wandb.init(project="UNet_v2_CheX",
            #    dir="./wandb",
    ...
    train_loader = get_loader_CheX(image_root, gt_root, batchsize=opt.batchsize, trainsize=opt.trainsize, augmentation=opt.augmentation)
