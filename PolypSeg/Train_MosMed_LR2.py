...
from utils.dataloader import get_loader, get_loader_MosMed, test_dataset
...
def train(train_loader, model, optimizer, epoch, test_path, state):
    ...
    if (epoch + 1) % 1 == 0:
        ...
        wandb.log({f"MosMed/mDice": test_macro_dice}, step=epoch)
        wandb.log({f"MosMed/mIoU": test_macro_miou}, step=epoch)
        wandb.log({f"MosMed/gDice": test_micro_dice}, step=epoch)
        wandb.log({f"MosMed/gIoU": test_micro_miou}, step=epoch)
        wandb.log({f"MosMed/Acc": test_micro_acc}, step=epoch)
...
if __name__ == '__main__':
    ...
    wandb.init(project="UNet_v2_MosMed",
            #    dir="./wandb",
    ...
    train_loader = get_loader_MosMed(image_root, gt_root, batchsize=opt.batchsize, trainsize=opt.trainsize, augmentation=opt.augmentation)
